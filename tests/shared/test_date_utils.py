...
class TestParseVietnameseDate:
    ...
    def test_ngay_nay_nam_truoc(self):
        ...
        # Handle Feb 29 case
        try:
            expected = datetime(today.year - 1, today.month, today.day).strftime(
                "%Y-%m-%d"
            )
        except ValueError:
            expected = datetime(today.year - 1, today.month, today.day - 1).strftime(
                "%Y-%m-%d"
            )
...
class TestResolveRelativeDates:
    ...
    def test_resolve_preserves_rest(self):
        ...
        text, date_str = resolve_relative_dates(
            "Coffee ngày mai tại The Coffee House 55k"
        )
...
class TestResolveRelativeDatesComplex:
    ...
    def test_resolve_ngay_nay_nam_truoc(self):
        ...
        today = datetime.now()
        try:
            expected_date = datetime(today.year - 1, today.month, today.day).strftime(
                "%Y-%m-%d"
            )
        except ValueError:
            expected_date = datetime(
                today.year - 1, today.month, today.day - 1
            ).strftime("%Y-%m-%d")

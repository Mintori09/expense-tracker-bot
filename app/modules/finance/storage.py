...
from app.core.database import Transaction, get_transactions, get_db_cursor
...
def get_transactions_by_period(period: str, year: int = None, month: int = None, chat_id: int = None) -> list[Transaction]:
    ...
    return [
        Transaction(
            id=row[0], date=row[1], merchant=row[2], amount=row[3], currency=row[4],
...
def export_period_to_excel(period: str, year: int = None, month: int = None) -> tuple[str, int]:
    ...
    if not transactions:
        ...
        df = pd.DataFrame(columns=[
            ...
            "Payment Method", "Description", "Source", "Confidence", "Needs Review"
        ])
